...
// Aktive IPv4-Netzwerke dieses Macs: [{ name, address, netmask, prefix, network, broadcast, networkInt, broadcastInt, mac, linkLocal }]
...
// Alle möglichen Geräte-Adressen im Netz (ohne Netz-/Rundruf-Adresse und ohne den Mac selbst); bei großen Netzen ein Fenster von höchstens max Adressen
...
// ctx: { ip, ifaces, gateway, route, probes: { ping, arp, udp, mdns, gatewayUdp }, terminal }
...
//   terminal { replied: bool } (Test über das Terminal, falls gemacht)
function diagnose(ctx) {
  const { ip, ifaces, probes } = ctx, checks = [], add = (id, level, title, detail) => checks.push({ id, level, title, detail: detail || "" });
  ...
  // --- einzelne Prüfpunkte ---
  add("net", ifaces.length ? "ok" : "fail", "Netzwerk des Macs", ifaces.length ? ifaces.map(label).join("; ") : "kein aktives WLAN/LAN mit IPv4-Adresse gefunden");
  add("target", target.ok ? "ok" : "fail", "Adresse des Pults", target.ok ? target.ip : target.reason);
  if (target.ok && !target.loopback) add("subnet", iface ? "ok" : "fail", "Gleiches Netz wie der Mac", iface ? "ja, über " + iface.name : "nein: die Adresse liegt in keinem der Netze dieses Macs" + (ctx.route && ctx.route.gateway ? " (Pakete gehen über den Router " + ctx.route.gateway + " ins Nirgendwo)" : ""));
  ...
  if (ctx.terminal) add("terminal", ctx.terminal.replied ? "ok" : "fail", "Vergleichstest über das Terminal", ctx.terminal.replied ? "Das Pult antwortet dem Terminal" : "auch das Terminal bekommt keine Antwort");

  // --- Urteil ---
  const gwFail = p.gatewayUdp && p.gatewayUdp.ok === false;
  let v;
  if (udp.reply) {
    ...
    v = { id: "no-network", level: "fail", title: "Der Mac ist mit keinem Netzwerk verbunden", text: "Es gibt kein aktives WLAN oder LAN mit einer IP-Adresse. Ohne Netzwerk kann die App das Pult nicht erreichen.", steps: ["WLAN einschalten und mit dem Netz verbinden, in dem auch das Pult hängt (oder das Netzwerkkabel anstecken).", "Bei direktem Kabel zwischen Mac und Pult: beiden Geräten feste Adressen im selben Netz geben (zum Beispiel Mac 192.168.1.10, Pult 192.168.1.20, Subnetzmaske 255.255.255.0)."], actions: [{ id: "retry", label: "Nochmal prüfen" }] };
  } else if (!target.loopback && !iface) {
    const near = ifaces[0];
    v = { id: "subnet", level: "fail", title: "Pult und Mac sind in verschiedenen Netzen", text: "Der Mac hängt im Netz " + cidr(near) + " (seine Adresse: " + near.address + "). Die Pult-Adresse " + target.ip + " liegt nicht in diesem Netz. Die Pakete kommen deshalb nie beim Pult an.",
      steps: ["Mac und Pult müssen am selben Router oder Switch hängen und Adressen im selben Netz haben. Beim Mac passt zum Beispiel " + near.network.replace(/\.0$/, ".") + "x mit einer freien Zahl statt x.", ...PULT_STEPS, "Stimmt die Adresse am Pult schon, dann ist der Mac im falschen WLAN. Im richtigen WLAN anmelden oder den Mac mit einem Kabel an den Router des Pults anschließen.", "Oder auf „Pult im Netz suchen“ drücken: die App findet das Pult im aktuellen Netz und trägt die richtige Adresse selbst ein."],
      actions: [{ id: "scan", label: "Pult im Netz suchen" }] };
  } else if (ctx.terminal && ctx.terminal.replied) {
  ...
  } else if (mdnsBlocked || gwFail || (errCodes.length && ping.ok === true)) {
    ...
    v = { id: "silent-osc", level: "warn", title: "Das Gerät ist da, sendet aber keine X32-Antwort", text: "Unter " + target.ip + " antwortet ein Gerät auf Ping, aber nicht auf die X32-Fernsteuerung (UDP Port 10023). Die App darf ins lokale Netz (sie hat Antworten anderer Geräte bekommen). Es ist also entweder nicht das Pult, oder das Pult ist gerade nicht bereit.",
